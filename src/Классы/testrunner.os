...
Функция ТестироватьФайл(Знач ФайлТеста, Знач ЛогФайлJUnit = Неопределено, Знач НаименованиеТестаДляЗапуска = Неопределено, 
	...
	НаборТестов.ЗагрузитьТесты(Пути);
...
Функция ТестироватьКаталог(Знач КаталогТестов, Знач ЛогФайлJUnit = Неопределено) Экспорт
	...
	КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.ЗапуститьКаталог;
...
Функция ПолучитьПараметрыЗапуска(МассивПараметров)
	...
	ИначеЕсли КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.Запустить Тогда
		НомерПараметраПутьКТестам = 1;
	ИначеЕсли КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.ЗапуститьКаталог Тогда
		НомерПараметраПутьКТестам = 1;
	...
	Если КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.Запустить Тогда
		...
		Если МассивПараметров.Количество() > НомерОчередногоПараметра Тогда
			...
			Если НРег(ИД_Теста) <> Перечисления.СтруктураПараметровЗапуска.Режим_ПутьЛогФайла Тогда
	...
	Если МассивПараметров.Количество() > НомерОчередногоПараметра и (КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.Запустить или КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.ЗапуститьКаталог ) Тогда
		Режим = НРег(МассивПараметров[НомерОчередногоПараметра]);
		Если Режим = Перечисления.СтруктураПараметровЗапуска.Режим_ПутьЛогФайла Тогда
...
Функция ОбработатьПараметрыЗапуска(МассивПараметров)
	...
	Если КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.Запустить Тогда
		...
		Возврат Истина;
	КонецЕсли;
	Если КомандаЗапуска = Перечисления.СтруктураПараметровЗапуска.ЗапуститьКаталог Тогда
...
Функция ВыполнитьТест(ОписаниеТеста, Сч)
	...
	Рефлектор.УстановитьОбъект(Тест);
	Успешно = Истина;
	Если Не Успешно Тогда
		Рез = ЗначенияСостоянияТестов.Сломался;
	Иначе
		
		Если Не Рефлектор.ЕстьПроцедура(ИмяМетода) Тогда
		...
		Иначе
			...
			ОписаниеТеста.УвеличитьВремяВыполнения(ВремяВыполнения);
...
Функция ПрочитатьТестовыеСлучаиОбъектаВызовомМетода(Знач ТестОбъект, Знач МенеджерТестирования)
	...
	Рефлектор.УстановитьОбъект(ТестОбъект);

	Если НЕ Рефлектор.ЕстьФункция("ПолучитьСписокТестов") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПрочитатьТестовыеСлучаиОбъектаИзАннотаций(Знач ТестОбъект, Аннотация)
	...
	Рефлектор.УстановитьОбъект(ТестОбъект);
	ТаблицаМетодов = Рефлектор.ПолучитьТаблицуМетодов(Аннотация);
	Если ТаблицаМетодов.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;

	МассивТестовыхСлучаев = ТаблицаМетодов.ВыгрузитьКолонку("Имя");
	Возврат МассивТестовыхСлучаев;
КонецФункции
...
Процедура Инициализация()
	...
	Пути = Новый Массив;
	НаборТестов = Новый НаборТестов;
	Рефлектор = Новый РефлекторОбъекта(Неопределено);
